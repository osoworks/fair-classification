        ...
        if self.train_multiple:
            ...
            for k in set(x_sensitive):
                ...
                if self.loss_function == "logreg":
                    ...
                    obj += sum(max_elemwise(0, 1 - multiply(y_k, X_k @ w[k]))) / num_all
            ...
            if self.loss_function == "logreg":
                ...
                obj += sum(max_elemwise(0, 1 - multiply(y, X @ w))) / num_all
            else:
                raise Exception("Invalid loss function")
            ...
            for attr in set(x_sensitive):  # there is only one boundary, so the results with this_group and other_group boundaries are the same
                ...
                for k in set(x_sensitive):
                    distances_boundary_dict[attr][k] = self.decision_function(
                        X[idx])  # apply same decision function for all the sensitive attrs because same w is trained for everyone
...
if __name__ == "__main__":
    ...
    # LinearClf 객체 생성
    clf = LinearClf(loss_function="logreg", lam=0.1, train_multiple=False)

    # 모델 훈련
    clf.fit(X, y, x_sensitive)

    # 훈련된 가중치 출력
    print("Trained weights:", clf.w)
